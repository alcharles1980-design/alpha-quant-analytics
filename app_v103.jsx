...
function BuildDataSetPage(p){
  ...
  return <div>
    ...
    <CollapseStage title="Derived: Daily Regime Features (Daily Correlation)" sub="day-over-day acceleration, 3-day ratios, compression, momentum, TP% trend" badge="12 LEADABLE" badgeColor={C.warn} badgeBg={C.warnDim}>
      <div style={{color:C.txt,fontSize:9,fontFamily:F,lineHeight:1.8}}>
        <div style={fS}>
          <p style={{marginBottom:6,color:C.accent,fontWeight:700}}>Summary: Multi-day regime detection features. Same concept as hourly regime features but across trading days. All LEADABLE -- computed from previous days' data using rolling buffers. Captures multi-day volatility trends, squeeze/breakout patterns, and TP% persistence.</p>
          <p style={{marginBottom:6,color:C.txtBright,fontWeight:700}}>Acceleration (day-over-day rate of change -- 4 features):</p>
          <p style={{marginBottom:2,paddingLeft:8}}><span style={{color:C.gold}}>day_atr_acceleration:</span> Change in daily avg ATR vs previous day. Captures multi-day regime shifts.</p>
          <p style={{marginBottom:2,paddingLeft:8}}><span style={{color:C.gold}}>day_realized_vol_acceleration:</span> Change in daily realized vol. Jump signals microstructure shift.</p>
          <p style={{marginBottom:2,paddingLeft:8}}><span style={{color:C.gold}}>day_volume_acceleration:</span> Day-over-day volume change. Surges precede volatility changes.</p>
          <p style={{marginBottom:2,paddingLeft:8}}><span style={{color:C.gold}}>day_reversal_acceleration:</span> Increasing = market becoming choppier over days.</p>
          <p style={{marginBottom:6,color:C.txtBright,fontWeight:700}}>Short vs Medium-Term (3-day lookback -- 2 features):</p>
          <p style={{marginBottom:2,paddingLeft:8}}><span style={{color:C.gold}}>day_atr_ratio_3d:</span> Today ATR / avg of previous 3 days. Breakout signal on daily timeframe.</p>
          <p style={{marginBottom:2,paddingLeft:8}}><span style={{color:C.gold}}>day_rv_ratio_3d:</span> Realized vol spike on multi-day scale.</p>
          <p style={{marginBottom:6,color:C.txtBright,fontWeight:700}}>Squeeze / Expansion (3 features):</p>
          <p style={{marginBottom:2,paddingLeft:8}}><span style={{color:C.gold}}>day_vol_compression_3d:</span> Min/max ATR over 3 days. Low = multi-day squeeze. Often precedes expansion.</p>
          <p style={{marginBottom:2,paddingLeft:8}}><span style={{color:C.gold}}>day_vol_expansion_streak:</span> Consecutive days of expanding ATR. 3+ = sustained increase.</p>
          <p style={{marginBottom:2,paddingLeft:8}}><span style={{color:C.gold}}>day_range_percentile_5d:</span> Where today's range sits vs last 5 days. 100% = widest day this week.</p>
          <p style={{marginBottom:6,color:C.txtBright,fontWeight:700}}>Trend / Persistence (3 features):</p>
          <p style={{marginBottom:2,paddingLeft:8}}><span style={{color:C.gold}}>day_drawdown_trend_3d:</span> Avg max drawdown over 3 days. Rising = increasing multi-day risk.</p>
          <p style={{marginBottom:2,paddingLeft:8}}><span style={{color:C.gold}}>day_price_momentum_3d:</span> Cumulative return over 3 days. Sustained trend vs range-bound.</p>
          <p style={{paddingLeft:8}}><span style={{color:C.gold}}>day_tp_trend_3d:</span> Average optimal TP% over last 3 days. Direct persistence: is the optimal TP% trending wider or tighter?</p>
        </div>
        <div style={cS}>{'// Daily regime: rolling buffers across trading days\nvar atrBuf = [], rvBuf = [], tpBuf = [];\nvar expansionStreak = 0;\nfor (var i = 0; i < joined.length; i++) {\n  // Acceleration: rate of change vs previous day\n  day_atr_acceleration = (myAtr - atrBuf[last]) / atrBuf[last] * 100;\n  \n  // 3-day ratio\n  day_atr_ratio_3d = myAtr / avg(atrBuf.slice(-3));\n  \n  // Compression over 3 days\n  day_vol_compression_3d = min(last3) / max(last3);\n  \n  // TP% persistence trend\n  day_tp_trend_3d = avg(tpBuf.slice(-3));\n  // If trending up: optimal TP% has been widening\n  // If trending down: optimal TP% has been tightening\n  \n  atrBuf.push(myAtr); tpBuf.push(bestTp);\n}'}</div>
      </div>
    </CollapseStage>
  </div>;
}
...
function CorrelationFinderPage(p){
  ...
  var run=async function(){
    ...
    try{
      ...
      var atrBuf=[],rvBuf=[],tvBuf=[],ddBuf=[],retBuf=[],volBuf=[],tiBuf=[],rrBuf=[];
  ...
  var runDaily=async function(){
    ...
    try{
      ...
      var dAtrBuf=[],dRvBuf=[],dVolBuf=[],dRngBuf=[],dRetBuf=[],dDdBuf=[],dRrBuf=[],dTpBuf=[];
      var dExpStreak=0;
