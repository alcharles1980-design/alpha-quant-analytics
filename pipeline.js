...
async function runScreener() {
  ...
  for (var ci = 0; ci < candidates.length; ci++) {
    ...
    // Daily Close to Next Day High % (full year data for statistical significance)
    var dcSwings = []; var dcByDow = {1:[],2:[],3:[],4:[],5:[]};
    for (var dci = 0; dci < allBars.length - 1; dci++) {
      if (allBars[dci].c > 0) {
        var dcPct = (allBars[dci + 1].h - allBars[dci].c) / allBars[dci].c * 100;
        dcSwings.push(dcPct);
        var dcDow = new Date(allBars[dci].date + 'T12:00:00Z').getDay(); // 1=Mon..5=Fri
    ...
    // Daily Low to Next Day High % — compute for multiple lookback windows
    var dlhPairs = []; // [{pct, dow}] — all swing values with day-of-week
    for (var dlhi = 0; dlhi < allBars.length - 1; dlhi++) {
      if (allBars[dlhi].l > 0) {
        ...
        dlhPairs.push({ pct: dlhPct, dow: dlhDow });
      }
    }
    // Helper: compute profile from a subset of pairs
    function _dlhProfile(pairs) {
      if (!pairs.length) return { avg: 0, min: 0, max: 0, std: 0, n: 0, mon: 0, tue: 0, wed: 0, thu: 0, fri: 0 };
      var sum = 0, mn = pairs[0].pct, mx = pairs[0].pct;
      var byDow = {1:[],2:[],3:[],4:[],5:[]};
      for (var j = 0; j < pairs.length; j++) {
        sum += pairs[j].pct;
        if (pairs[j].pct < mn) mn = pairs[j].pct;
        if (pairs[j].pct > mx) mx = pairs[j].pct;
        if (byDow[pairs[j].dow]) byDow[pairs[j].dow].push(pairs[j].pct);
      }
      var avg = sum / pairs.length;
      var vr = 0; for (var j = 0; j < pairs.length; j++) vr += (pairs[j].pct - avg) * (pairs[j].pct - avg);
      var std = Math.sqrt(vr / pairs.length);
      var dowAvg = {};
      for (var dw2 = 1; dw2 <= 5; dw2++) {
        var darr = byDow[dw2];
        if (darr && darr.length > 0) { var ds = 0; for (var j = 0; j < darr.length; j++) ds += darr[j]; dowAvg[dowNames[dw2]] = Math.round(ds / darr.length * 1000) / 1000; }
        else dowAvg[dowNames[dw2]] = 0;
      }
      var p = { avg: Math.round(avg * 1000) / 1000, min: Math.round(mn * 1000) / 1000, max: Math.round(mx * 1000) / 1000, std: Math.round(std * 1000) / 1000, n: pairs.length };
      for (var k in dowAvg) p[k] = dowAvg[k];
      return p;
    }
    var dlhWindows = [5, 10, 30, 60, 90, 180, 252];
    var dailyLowHigh = {};
    for (var wi = 0; wi < dlhWindows.length; wi++) {
      var w = dlhWindows[wi];
      var subset = dlhPairs.length > w ? dlhPairs.slice(-w) : dlhPairs;
      dailyLowHigh[String(w)] = _dlhProfile(subset);
    }
  ...
  for (var ri = 0; ri < results.length; ri++) {
    ...
    try {
      ...
      if (curStreak > 0) { olapStreakSum += curStreak; olapStreakCount++; }
      var avgOlap = olapCount > 0 ? olapSum / olapCount : 0;
      var gapPct = olapCount > 0 ? gapCount / olapCount * 100 : 0;
      var avgStreak = olapStreakCount > 0 ? olapStreakSum / olapStreakCount : 0;
      var hrOlapArr = [];
      for (var oh2 = 4; oh2 < 20; oh2++) {
        var hd = hrOlap[oh2];
      ...
      // Cycle Density computation — simulate grid at 4 TP% levels
      var cdLevels = [];
      var cdPcts = [0.0005, 0.001, 0.0015, 0.002]; // 0.05%, 0.10%, 0.15%, 0.20%
      for (var cpi = 0; cpi < cdPcts.length; cpi++) {
        var cdPct = cdPcts[cpi];
        var tpDollar = Math.ceil(res.price * cdPct * 100) / 100; // round up to whole cent
        if (tpDollar < 0.01) tpDollar = 0.01;
        var tpCents = Math.round(tpDollar * 100);
        var effPct = tpDollar / res.price;
        var cdTotal = 0;
        var cdHr = {}; for (var cdh = 4; cdh < 20; cdh++) cdHr[cdh] = 0;
        for (var cdi = 0; cdi < dbrKeys.length; cdi++) {
          var cdb = dayBarsForReturns[dbrKeys[cdi]]; if (cdb.length < 10) continue;
          var cdHeld = {};
          for (var cdbi = 0; cdbi < cdb.length; cdbi++) {
            var cdBar = cdb[cdbi];
            var cdLowC = Math.round(cdBar.l * 100);
            var cdHighC = Math.round(cdBar.h * 100);
            ...
            for (var cdk = 0; cdk < cdKeys.length; cdk++) {
              var cdLv = parseInt(cdKeys[cdk]);
              if (cdLv + tpCents <= cdHighC) { cdTotal++; if (cdHr[cdBHr] !== undefined) cdHr[cdBHr]++; delete cdHeld[cdLv]; }
            }
            // New buys: from entry point down to low
            var cdEntry = cdBullish ? cdOpenC - 1 : cdHighC - 1;
            for (var cdLv2 = cdEntry; cdLv2 >= cdLowC; cdLv2--) { if (cdHeld[cdLv2] === undefined) cdHeld[cdLv2] = true; }
          }
        }
        var cdHrArr = [];
        for (var cdh2 = 4; cdh2 < 20; cdh2++) cdHrArr.push(cdHr[cdh2]);
        cdLevels.push({ pct: cdPct, tp: tpDollar, eff: Math.round(effPct * 100000) / 100000, cycles: cdTotal, hr: cdHrArr });
      }
      res.cycle_density = JSON.stringify({ levels: cdLevels, days: dbrKeys.length, bars: bars5.length });
      ...
      var hdKeys = Object.keys(hrDayData);
      for (var hdi = 0; hdi < hdKeys.length; hdi++) {
        var hd = hrDayData[hdKeys[hdi]];
        if (hd.high <= 0 || hd.low <= 0 || hd.high === -Infinity) continue;
        var hAtr = (hd.high - hd.low) / hd.low * 100;
        if (!hrAtrSums[hd.hr]) { hrAtrSums[hd.hr] = 0; hrAtrCounts[hd.hr] = 0; }
        hrAtrSums[hd.hr] += hAtr;
        hrAtrCounts[hd.hr]++;
      }
      for (var h = 4; h < 20; h++) {
        hourlyAtr[h] = hrAtrCounts[h] ? Math.round(hrAtrSums[h] / hrAtrCounts[h] * 1000) / 1000 : 0;
      }
      ...
      for (var hh = 4; hh < 20; hh++) {
        var hourDayAtrs = [];
        var dkArr = Object.keys(dayBars);
        for (var dki2 = 0; dki2 < dkArr.length; dki2++) {
          var dayB = dayBars[dkArr[dki2]];
          // Filter bars for this hour using ET
          var hBars = [];
          for (var bi8 = 0; bi8 < dayB.length; bi8++) {
            var bTime2 = etFmt2.format(new Date(dayB[bi8].t));
            var bH2 = parseInt(bTime2.split(':')[0]) || 0;
            if (bH2 === hh) hBars.push(dayB[bi8]);
          }
          if (hBars.length >= 2) {
            var hHigh = -Infinity, hLow = Infinity;
            for (var bi9 = 0; bi9 < hBars.length; bi9++) {
              if (hBars[bi9].h > hHigh) hHigh = hBars[bi9].h;
              if (hBars[bi9].l < hLow) hLow = hBars[bi9].l;
            }
            var hOpen = hBars[0].o;
            if (hOpen > 0) hourDayAtrs.push(((hHigh - hLow) / hOpen) * 100);
          }
        }
        var avgHAtr = 0;
        if (hourDayAtrs.length > 0) { for (var j = 0; j < hourDayAtrs.length; j++) avgHAtr += hourDayAtrs[j]; avgHAtr /= hourDayAtrs.length; }
        hourlyAtr[hh] = Math.round(avgHAtr * 1000) / 1000;
      }
...
async function main() {
  ...
  if (mode === 'backfill') {
    ...
    try { await runBackfill(tickers, startDate, endDate, skipExisting); } catch (e) {
      console.error('BACKFILL CRASHED:', e.message, e.stack);
    }
  } else if (mode === 'nightly') {
    try { await runNightly(tickers); } catch (e) {
      console.error('NIGHTLY CRASHED:', e.message, e.stack);
      await reportProgress({ mode: 'nightly', ticker: 'ALL', status: 'error', progress_pct: 0, message: 'Crashed: ' + e.message });
    }
  } else if (mode === 'autotune') {
    try { await runAutotune(tickers); } catch (e) {
      console.error('AUTOTUNE CRASHED:', e.message, e.stack);
    }
    ...
    try { await runHourly(tickers); } catch (e) {
      console.error('HOURLY CRASHED:', e.message, e.stack);
      await reportProgress({ mode: 'hourly', ticker: 'ALL', status: 'error', progress_pct: 0, message: 'Crashed: ' + e.message });
    }
  }
